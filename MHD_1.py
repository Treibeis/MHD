...
def locus():
	...
	print('locus_s1(lam, t, x0, xm, l, d = {}, n, k = 0, f = 1, mindx, m0)\n')
	#print('locus_s11(lam, t, xs, xm, l, d, h, n, mindx, m0)\n')
	print('locus_s2(lam, t, xsu/d, xm, l, d, n, k, f, mindx, m0)\n')
...
def shock2_(x, l, lam):
	...
	m1 = l[0]-x
	X = 1.0
	A = (2.0-GAMMA)/(GAMMA-1.0)/b1
	B = (m1**2/2+GAMMA*(1.0+1.0/b1)/(GAMMA-1.0))
	C = -(GAMMA+1.0)*m1**2/2.0/(GAMMA-1.0)
	delta = B**2-4*A*C
	if delta<0:
		print("There is no roots")
		return []
	if (GAMMA+2.0/b1-m1**2)<0:
		print('There is no shock here.')
		return []
	r = np.roots([A, B, C])
	if r[0]>r[1]:
		j = r[1]
	...
	if ((X>=1)or(X<0)):
		...
		return []
	print('X=', X)
	T = (X-1.0)*(m1**2/X-(1.0+X)/b1)/X+1.0/X
	if T<0:
...
def locus_s1(lam, t, x0, xm, l, d = {}, n = 0, k = 0, f = 1, mindx = 1.0/10**5, m0 = 10000):
	...
	if t==-1:
		for i in range(n+1):
			...
			do = A.SIS(b)
			if k==0:
				l = shock2(b, do['v-a'], lam)
			else:
				l = shock2_(b, do['v-a'], lam)
			if l==[]:
				print(b)
				b = b + step
				continue
...
def locus_s2(lam, t, xs, xm, l, d = {}, n = 0, k = 0, f = 1, mindx = 1.0/10**5, m0 = 10000):
	...
	if l[1]/l[0] > 10*n:
		...
		if (t==2)or(t==3):
			for i in range(n+1):
				step = B*b-b
				l = locus_s1(lam, t, b, xm, [xs, 1], d, 0, k, f, mindx, m0)
				if l['v'] == []:
					b = b+step
					continue
				else:
